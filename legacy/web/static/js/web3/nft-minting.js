...
class NFTMinting {
    ...
    async loadContractInfo() {
        try {
            ...
            if (data.success && data.contract) {
                ...
                console.log('Contract info loaded:', this.contractAddress);
            } else {
                console.warn('Contract not ready:', data.message);
            }
        } catch (error) {
    ...
    async checkMintStatus(traderAddress) {
        ...
        try {
            const response = await fetch(`/api/nft/mint-status/${traderAddress}`);
            const data = await response.json();
